...
def compute_clm(pl_module, batch):
    ...
    acc = getattr(pl_module, f"{phase}_clm_accuracy")(
        ret["clm_logits"], ret["clm_labels"]
    )
    pl_module.log(f"clm/{phase}/loss", loss)
    pl_module.log(f"clm/{phase}/accuracy", acc)
...
def compute_tat_itm(pl_module, batch):
    pos_len = len(batch["context_ids"]) // 2
    neg_len = len(batch["context_ids"]) - pos_len
    itm_labels = torch.cat([torch.ones(pos_len), torch.zeros(neg_len)]).to(
        pl_module.device
    )
    itm_labels = itm_labels[torch.randperm(itm_labels.size(0))]
...
def compute_tat_itm_wpa(pl_module, batch):
    pos_len = len(batch["context_ids"]) // 2
    neg_len = len(batch["context_ids"]) - pos_len
    itm_labels = torch.cat([torch.ones(pos_len), torch.zeros(neg_len)]).to(
        pl_module.device
    ...
    pl_module.log(f"itm_wpa/{phase}/loss", loss)
    pl_module.log(f"itm_wpa/{phase}/wpa_loss", wpa_loss)
    pl_module.log(f"itm_wpa/{phase}/accuracy", acc)
